...
function Heartbeat.editor.drawEditor()
	...
	if (Heartbeat.editor.isActive) then
		...
		if (Heartbeat.editor.mode == "tile" or Heartbeat.editor.snapToGrid) then
...
function Heartbeat.editor.handleInput(key)
	...
	if (key == 1) then
		...
		if (Heartbeat.editor.mode == "entity") then
			...
			if (not Heartbeat.editor.snapToGrid) then
...
function Heartbeat.getItem(x, y)
	local checker = {
		x = x,
		y = y,
		width = 1,
	...
	for i=1,#Heartbeat.items do
		if (Heartbeat.checkEntityCollision(Heartbeat.items[i], checker)) then
			return Heartbeat.items[i]
		end
	end
